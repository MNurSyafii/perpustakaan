...
<div class="container mt-3 mt-md-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="fw-bold mb-0">Edit Relasi Kategori Buku</h2>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('kategoribuku_relasi.update', $relasi->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="buku_id" id="buku_id" class="form-select" required>
                        ...
                        @foreach ($buku as $item)
                            <option value="{{ $item->id }}" {{ (old('buku_id', $relasi->buku_id) == $item->id) ? 'selected' : '' }}>
                                {{ $item->judul }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="kategori_id" id="kategori_id" class="form-select" required>
                        ...
                        @foreach ($kategori as $item)
                            <option value="{{ $item->id }}" {{ (old('kategori_id', $relasi->kategori_id) == $item->id) ? 'selected' : '' }}>
                                {{ $item->nama_kategori }}
                            </option>
                        @endforeach
                    </select>
